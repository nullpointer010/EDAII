...
public class Ciudad {
	...
	public void Greedy() 
	{
		...
		for (Distrito distrito : distritos) 
		{
			for(int i=0; i < distrito.numeroDias(); i++) // Candidatos
			{   // La primera condición es para evitar el arrayoutbound
				...
				if(distrito.get(i) > distrito.get(i+1)) // Funcion de factibilidad
				{
					casoMayor = distrito.get(i);
					dia = i;
					break;
	...
	public void GreedyOrdenado() 
	{
		...
		for (Distrito distrito : distritos) 
		{
			//Primero ordenamos los elementos
			Utilities.ordenarArray(distrito.getListaCasos(), 0, distrito.numeroDias());
			
			// El primer elemento siempre será el de mayor casos en la lista por lo que:
			casoMayor = distrito.get(0);
	...
	/*** APARTADO 2B ***/
	public void GreedySuavizar() 
	{
		//Ordenación O(n log n)
		int[] elementos = new int[2];

		for (Distrito distrito : distritos) // Mientras queden distritos
		{
			//Selecciona un distrito.
			//Copia los valores.
			int [] copiaCasos = distrito.getListaCasos().clone();
			for(int i=0; i < distrito.numeroDias()+1; i++)
			{
				elementos = seleccionarObjetoGreedy(i, copiaCasos); //Selecciona elementos greedy y obtiene su factibilidad
				elementos[0] += distrito.get(i); // Se añade la media del elemento actual
				elementos[1] += 1;
				
				distrito.set(elementos[0]/elementos[1], i); //Aplicamos

			}
		}
		System.out.println("Para comprobar los resultados de este algoritmo en ejecución- Véase los valores de los"
				+ "\nDistrito en mostrar resultados. Serán diferentes a los de carga."
				+ "\nSe ha generado resultados más detallados en el ApartadoB de Resultados ");
	}
	
	public int [] seleccionarObjetoGreedy(int n, int [] casos) 
	{
		int i;
		int[] elementos = new int[2];
		elementos[0] = 0;
		...
		for(i = n+1; i <= n+4; i++) //Derecha // Si sumamos 1 pues es hasta 4
		{
			if(i < casos.length) {
				elementos[0] += casos[i]; 
				elementos[1] += 1;
